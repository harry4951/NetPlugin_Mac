...
ReadNetwork::ReadNetwork(QueueMutexUdp<float>& buffer)
: Thread ("networkThread"), queue_(buffer),_count(0),_is_timer_running(false),_is_current_packet(true),PacketArrivalIntervalLimit(150)
{
    start_sock();
    //thread ReadNetwork_thread(&ReadNetwork::run);
